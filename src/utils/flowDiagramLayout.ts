...
import type { DataTypeId, AssetId } from "../graph/ids";
import type { RollupsApi } from "../engine/rollups";

export type FlowLayout = ReturnType<RollupsApi["flowLayoutForSystem"]>;
type LayoutAsset = FlowLayout["stages"][number]["nodes"][number];
type LayoutActor = FlowLayout["ingressActors"][number]["actor"];
type DiagramReference = LayoutAsset | LayoutActor;
...
interface PositionedNode extends Rectangle {
  kind: "actor" | "asset";
  ref: DiagramReference;
  branch?: boolean;
}

export interface DiagramNode extends PositionedNode {
  id: string;
}
...
export interface DiagramEdge extends RawDiagramEdge {
  path: string;
  start: Point;
  end: Point;
}
...
export interface DataFlowDiagram {
  ...
  nodes: DiagramNode[];
  edges: DiagramEdge[];
  categoryLabels?: Array<{ label: string; x: number; y: number }>;
}
...
function columnOf(items: ColumnItem[], kind: DiagramColumn["kind"]): DiagramColumn {
  return { kind, items };
}
...
function anchor(a: Rectangle, b: Rectangle): [Point, Point] {
...
function perp(p1: Point, p2: Point, offset: number): Point {
  const dx = p2.x - p1.x, dy = p2.y - p1.y;
  const len = Math.hypot(dx, dy) || 1;
  return { x: (-dy / len) * offset, y: (dx / len) * offset };
}

function curve(p1: Point, p2: Point, offset: number): string {
...
// each other.
function fanOutEdges(rawEdges: RawDiagramEdge[], positions: Record<string, PositionedNode>): DiagramEdge[] {
  const groups: Record<string, RawDiagramEdge[]> = {};
  rawEdges.forEach((e) => {
...
export function buildDataFlowDiagram(layout: FlowLayout): DataFlowDiagram {
...
const CONTROL_CATEGORY_BY_KIND: Record<string, string> = {
...
interface MatrixCategoryBreak {
  label: string;
  rowIndex: number;
}

type MatrixCell = FlowLayout["controlPlaneEdges"][number] | { note: string };

export interface ControlPlaneMatrix {
  width: number;
  height: number;
  rows: MatrixRow[];
  cols: MatrixColumn[];
  categoryBreaks: MatrixCategoryBreak[];
  cellByKey: Record<string, MatrixCell>;
  margin: number;
  rowLabelW: number;
  colW: number;
  rowH: number;
...
export function buildControlPlaneMatrix(
  layout: FlowLayout,
  getAssetById: (id: AssetId) => LayoutAsset | null | undefined,
): ControlPlaneMatrix | null {
  ...
  const groups: Record<string, LayoutAsset[]> = {};
  branches.forEach((b) => {
  ...
  const deployTargets: Record<string, Array<{ to: AssetId; note: string }>> = {};
  ...
  const cellByKey: Record<string, MatrixCell> = {};
  (layout.controlPlaneEdges || []).forEach((f) => {
