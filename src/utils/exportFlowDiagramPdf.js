...
export function buildDataFlowRows(layout, getAssetLabel) {
  const rows = [];
...
export function buildControlPlaneRows(layout, getAssetLabel) {
...
const TABLE_PAGE = [842, 595]; // A4 landscape, pt
...
function drawRows(doc, rows, title) {
...
function drawDiagramHeader(doc, title, dataTypeLabel) {
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.setTextColor(20, 20, 30);
...
export async function exportFlowDiagramPdf({ dataSvgEl, dataDiagram, controlSvgEl, controlDiagram, system, dataTypeLabel, layout, getAssetLabel }) {
