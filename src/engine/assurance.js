...
// judgments.
import { MATURITY_STAGES, EVIDENCE_TYPES } from "../graph/nodes/taxonomy";
...
export function maturityScore(stage) {
  return MATURITY_SCORE[stage] ?? 0;
}

export function evidenceBaseConfidence(type) {
  return EVIDENCE_CONFIDENCE[type] ?? 0;
}

export function meetsMaturity(actual, required) {
  return MATURITY_STAGES.indexOf(actual) >= MATURITY_STAGES.indexOf(required);
}

export function meetsEvidence(actual, required) {
...
// consume.
export function blendAssurance({ maturityStage, evidenceConfidence, effectivenessPct }) {
  return (
