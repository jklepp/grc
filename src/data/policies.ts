...
export interface PolicyClassificationLevel {
...
export interface AuthoredPolicy {
  ...
  domains: string[];
  controlIds: string[];
  statements: Array<{ title: string; detail: string }>;
  standards: Array<{ title: string; detail: string }>;
  roles: Array<{ role: string; responsibility: string }>;
  relatedPolicyIds?: string[];
  classificationLevels?: PolicyClassificationLevel[];
  created: string;
  lastReviewed: string;
}

type PolicyDefinition = Omit<AuthoredPolicy, "created" | "lastReviewed">;

type ScfControl = (typeof scf.controls)[number] & { frameworks: Record<string, string[]> };
const CONTROL_BY_ID: Record<string, ScfControl | undefined> = Object.fromEntries(
  scf.controls.map((control) => [control.id, control as ScfControl]),
);

export function getPolicyControls(controlIds: readonly string[]): ScfControl[] {
  return controlIds.map((id) => CONTROL_BY_ID[id]).filter((control): control is ScfControl => control !== undefined);
}

// Unique, sorted clause citations for one framework across every control a policy maps to.
export function getFrameworkClauses(controlIds: readonly string[], standard: string): string[] {
...
export function getApplicableStandards(controlIds: readonly string[]): string[] {
...
export const STANDARD_ABBR: Record<string, string> = { "SOC 2": "SOC2", "ISO 27001": "27001", "ISO 27017": "27017", "ISO 27018": "27018", "ISO 27701": "27701", "PCI DSS": "PCI", HIPAA: "HIPAA", "ISO 42001": "42001" };
...
export const POLICY_TIERS = { CORE: "Core", EXTENDED: "Extended" } as const;
...
// than no proof.
const POLICY_REVIEW_DATES: Record<string, { created: string; lastReviewed: string }> = {
...
const POLICY_DEFS: PolicyDefinition[] = [
  {
...

  {
    ...
    statements: [
      ...
      { title: "Personal devices need approval first", detail: "Personal phones or laptops may only access ACME email or data after enrolling in Intune under the Access Control Policy. Unmanaged personal devices are not permitted to connect." },
    ...
    relatedPolicyIds: ["access-control", "data-classification"],
  },

  {
    ...
    purpose: "Your login is the front door to ACME's systems, and the device you use is how that door is reached. This policy explains what we expect from you, what we do automatically to keep accounts safe, and what's required before a company or personal device can touch ACME email, chat, or files.",
    scope: "All employees, contractors, and system/service accounts accessing ACME systems, and every mobile device — company-issued or personal — used to access ACME email, apps, or data.",
    statements: [
      ...
      { title: "Enroll the device before you connect", detail: "Every device accessing ACME email or apps — company or personal — must be enrolled in Microsoft Intune first. Unmanaged devices are blocked automatically. Remote access also routes through Zscaler and requires the device to pass an Intune compliance check (screen lock, encryption, current OS) before reaching ACME data." },
      { title: "Keep basic device security on", detail: "A screen lock (PIN, password, or biometric) and current OS updates are required. Intune checks for this automatically and will block access if a device falls out of compliance. A modified or jailbroken device cannot enroll or connect, personal or company-owned." },
      { title: "ACME can wipe the work part", detail: "On a personal device, Intune only manages and can remotely wipe ACME's app data (email, files) — not your personal photos or apps. On a lost or stolen ACME-owned device, the whole device may be wiped. Report a lost or stolen phone that has ACME data or app access to IT Security immediately." },
    ],
    standards: [
      ...
      { title: "Container encryption & remote purge", detail: "ACME managed apps use app-level containerization and encryption on BYOD devices so ACME data is cryptographically separated from personal data. Selective wipe (ACME data only) is used for BYOD; full device wipe is reserved for company-owned devices." },
    ],
    roles: [
      { role: "Employees", responsibility: "Protect credentials, use MFA, enroll devices, keep them updated, report suspicious login prompts and lost/stolen devices immediately" },
      { role: "Managers", responsibility: "Approve or deny access requests for their team; flag role changes promptly" },
      { role: "IT Security", responsibility: "Provisions/de-provisions accounts, runs access reviews, administers Entra ID/Okta, Intune compliance, and remote wipes" },
...

  {
    ...
    tier: POLICY_TIERS.CORE,
    domains: ["Privacy"],
    controlIds: ["PRI-01","PRI-02","PRI-04","PRI-05","PRI-06"],
...

  {
    ...
    controlIds: ["NET-01", "NET-03", "NET-06", "NET-14", "NET-17", "WEB-01", "WEB-03"],
    purpose: "This policy covers how ACME protects its network and how you connect securely from the office, home, or on the road — including the public-facing web apps that sit on the perimeter.",
    scope: "All connections to ACME's network and internal systems, from any location, and every customer-facing web application ACME exposes.",
    statements: [
      ...
      { title: "Public-facing apps sit behind a WAF", detail: "Customer-facing web applications sit in a DMZ behind Cloudflare's WAF and CDN, with TLS enforced end-to-end — they are never exposed directly to the internet." },
    ...
    standards: [
      ...
      { title: "Web application protection", detail: "Customer-facing applications sit behind Cloudflare's WAF and CDN, with TLS enforced and change detection monitoring for unauthorized website modifications." },
    ],
    roles: [
      { role: "Employees", responsibility: "Keep Zscaler active, use approved communication tools" },
      { role: "IT Security", responsibility: "Administers Zscaler policy, network segmentation, DLP rules, and the WAF/CDN perimeter" },
    ],
    relatedPolicyIds: ["change-configuration", "data-classification", "security-monitoring"],
  },

  {
    ...
    purpose: "This policy explains how changes to ACME's systems get proposed, reviewed, and made safely — and how a new cloud or SaaS service gets approved before ACME data ever touches it, so we don't end up with unmanaged shadow IT.",
    scope: "All changes to production systems, network configuration, and business-critical applications, and all cloud infrastructure (primarily Microsoft Azure) and SaaS applications used to run or support ACME's business.",
    statements: [
    ...
    relatedPolicyIds: ["vulnerability-patch", "secure-development", "network-remote-access", "third-party-risk"],
  },

  {
    ...
    controlIds: ["VPM-01","VPM-05","VPM-06","VPM-07"],
...

  {
    ...
    standards: [
      ...
      { title: "Web application protection", detail: "Customer-facing web applications sit behind Cloudflare's WAF and CDN as required by the Network & Remote Access Policy, with TLS enforced and change detection monitoring for unauthorized modifications." },
    ...
    relatedPolicyIds: ["vulnerability-patch", "change-configuration", "third-party-risk", "network-remote-access"],
  },

  {
    ...
    controlIds: ["TPM-01","TPM-04","TPM-08","TPM-11"],
...

  {
    ...
    controlIds: ["IRO-01","IRO-02","IRO-04","IRO-06","IRO-10"],
...

  {
    ...
    relatedPolicyIds: ["incident-response", "network-remote-access", "vulnerability-patch", "acceptable-use-asset"],
  },

  {
    ...
    controlIds: ["PES-01", "PES-03", "MNT-01", "MNT-05"],
    purpose: "This policy covers physical access to ACME offices and any facility housing ACME systems — badges, visitors, keeping equipment physically safe, and how maintenance on that equipment is scheduled and logged.",
    scope: "All ACME offices, data closets, and any alternate/remote work site where ACME equipment is used, and any vendor or staff performing maintenance on ACME systems.",
    statements: [
      ...
      { title: "Remote maintenance is logged", detail: "Any remote maintenance session on ACME infrastructure — including by a vendor — is authorized in advance and logged. Patching and maintenance happen in communicated windows, not ad hoc." },
    ],
    standards: [
      ...
      { title: "Maintenance windows", detail: "Scheduled maintenance occurs in defined windows communicated to affected stakeholders in advance. Remote maintenance tools are approved, and sessions are authorized before they start." },
    ],
    roles: [
      { role: "Facilities/IT", responsibility: "Administers badge access, visitor management, environmental monitoring, and maintenance windows" },
      { role: "All Employees", responsibility: "Follow badge and visitor procedures; practice clean-desk habits" },
    ],
    relatedPolicyIds: ["acceptable-use-asset", "business-continuity", "change-configuration"],
  },

  {
    ...
    controlIds: ["HRS-01","HRS-04","HRS-09","HRS-11"],
...

  {
    ...
    controlIds: ["AAT-01", "AAT-07", "AAT-10", "AAT-16", "AAT-17"],
    ...
    statements: [
      ...
      { title: "New AI tools go through the same approval as any new tool", detail: "Want to use a new AI tool or vendor feature for ACME work? It goes through the same SaaS approval process in the Change & Configuration Management Policy as any other cloud tool, plus a privacy check if it touches personal data." },
    ...
    relatedPolicyIds: ["data-classification", "third-party-risk", "secure-development", "change-configuration"],
  },
];
...
export const POLICIES: AuthoredPolicy[] = POLICY_DEFS.map((p) => {
