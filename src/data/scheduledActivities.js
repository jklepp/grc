...
export const PERIODS_PER_YEAR = { Monthly: 12, Quarterly: 4, Annual: 1 };
...
export function deriveStatus(instance, today = new Date(TODAY)) {
...
export function currentPeriod(frequency, today = TODAY) {
...
export function activitiesForFrequency(frequency) {
  return SCHEDULED_ACTIVITIES.filter((a) => a.frequency === frequency);
}

export function statusCountsForPeriod(frequency, period) {
  const counts = { completed: 0, in_progress: 0, upcoming: 0, at_risk: 0, overdue: 0 };
