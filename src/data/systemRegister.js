...
export const SYSTEMS = [
  {
    ...
    mission: "Centralizes customer, product-usage, and financial transaction data from ACME's production systems into a single analytics store, giving Finance, Sales, and Product one source of truth for revenue and customer reporting.",
    boundary: "Runs entirely within a dedicated AWS production account, network-isolated from ACME's corporate network and other AWS accounts. The boundary covers the data warehouse, its ETL pipeline, and the BI access layer; the production application databases that feed it are separately owned systems outside this boundary.",
    connections: [
      "Inbound: nightly ETL feed from production application databases (AWS Glue)",
      "Outbound: read-only access for ACME's internal BI/analytics tool",
      "No direct connection to the public internet",
    ],
    roles: [
      { role: "System Owner", assignment: "Data Platform Team" },
      { role: "Technical Administrator", assignment: "Data Platform Team (AWS)" },
      { role: "Security Owner", assignment: "IT Security" },
      { role: "Data Owner", assignment: "Finance & Analytics Leadership" },
    ],
    syncSource: "vanta",
  ...
  },
  {
    ...
    mission: "ACME's customer support ticketing system, used by the support organization to receive, triage, and resolve customer issues that may include personal and health-related information customers share when describing a problem.",
    boundary: "A SaaS application (Zendesk) hosted outside ACME's own infrastructure. The boundary covers ACME's tenant configuration, agent access, and the data flowing into and out of that tenant; Zendesk's underlying infrastructure is covered by Zendesk's own SOC 2 report, not this plan.",
    connections: [
      "Inbound: customer-submitted tickets via web form and email",
      "Access: support agents authenticate via SSO (Okta)",
      "Integration: ticket data referenced read-only by internal support dashboards",
    ],
    roles: [
      { role: "System Owner", assignment: "Support Operations" },
      { role: "Technical Administrator", assignment: "IT Security (SaaS administration)" },
      { role: "Security Owner", assignment: "IT Security" },
      { role: "Data Owner", assignment: "Support Operations" },
    ],
    syncSource: "vanta",
    lastSynced: "8 min ago",
    oktaEnforced: "compliant",
  ...
  },
  {
    ...
    mission: "An on-premises database supporting legacy invoicing and billing operations for customers not yet migrated to ACME's current billing platform.",
    boundary: "Runs on-premises in ACME's data center on ACME-owned and -managed hardware. The boundary covers the database server and the internal billing application that queries it; it has no direct external network exposure.",
    connections: [
      "Inbound: internal billing application, reachable only from the corporate network",
      "Outbound: none — no external network connections",
      "Scheduled: nightly backup job to on-premises backup storage",
    ],
    roles: [
      { role: "System Owner", assignment: "Finance Systems" },
      { role: "Technical Administrator", assignment: "IT Infrastructure" },
      { role: "Security Owner", assignment: "IT Security" },
      { role: "Data Owner", assignment: "Finance Systems" },
    ],
