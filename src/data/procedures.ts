...
// `domains` list is a partition of the 20 ACME control domains — every
// domain appears in exactly one SOP, so every in-scope common control
...
export interface ProcedureFileEvidence {
  type: "file";
  label: string;
  accept?: string;
}

export interface ProcedureFieldsEvidence {
  type: "fields";
  label: string;
  fields: Array<{ key: string; label: string; kind: "number" }>;
  reconcile?: { formula: string; check: (values: Record<string, string | number | undefined>) => boolean };
  summaryKey?: string;
}

export interface ProcedureTicketsEvidence {
  type: "tickets";
  label: string;
  pattern?: RegExp;
  patternHint?: string;
  expectedFromStep?: number;
  expectedLabel?: string;
}

export interface ProcedureVerifyEvidence {
  type: "verify";
  label: string;
  expectedFromStep?: number;
  expectedLabel?: string;
...
export type ProcedureStepEvidence = ProcedureFileEvidence | ProcedureFieldsEvidence | ProcedureTicketsEvidence | ProcedureVerifyEvidence;

export interface AuthoredProcedureStep {
  title: string;
  detail: string;
  controls?: string[];
  group?: string;
...
export interface AuthoredProcedure {
  ...
  purpose: string;
  steps: AuthoredProcedureStep[];
  stepGroups?: Array<{ id: string; title: string; blurb: string }>;
  controlIds: string[];
  uncitedControlIds: string[];
}

type ProcedureDefinition = Omit<AuthoredProcedure, "controlIds" | "uncitedControlIds">;

const PROCEDURE_DEFS: ProcedureDefinition[] = [
  {
    ...
    steps: [
      { title: "Classify before you provision", detail: "Confirm the data store's classification tier against the Data Classification & Handling Policy before it's built — the tier decides what's required below, not the other way around. A tier that changes later triggers a reclassification of every control on this list.", controls: ["DCH-02"] },
      { title: "Label it so it's traceable", detail: "Apply the Microsoft Purview sensitivity label matching the data's classification tier so downstream tools (DLP, access policies) can act on it automatically instead of relying on someone remembering.", controls: ["DCH-02", "DCH-01"] },
      { title: "Encrypt at rest to the floor standard", detail: "AES-256 (or equivalent) at rest is the non-negotiable floor for Confidential and Restricted data — this is checked, not assumed, at the Verify step below.", controls: ["CRY-05"] },
      { title: "Encrypt in transit end-to-end", detail: "TLS 1.2+ is enforced on every segment carrying Confidential or Restricted data, including internal service-to-service connections and wireless access — internal traffic isn't treated as implicitly trusted.", controls: ["CRY-03"] },
      { title: "Own and rotate keys in Key Vault", detail: "Encryption keys are generated and held in Azure Key Vault, backed by a managed PKI for certificate-based trust — no production key is ever generated or stored anywhere else, including an engineer's machine. Data encryption keys rotate at least every 12 months; secrets rotate at least every 90 days. Rotation events are logged for audit. The same rule applies to application secrets generally: a database credential, API key, or connection string is never embedded in source code, a container image, a configuration file, or a CI/CD pipeline variable where a managed identity or a Key Vault reference is technically feasible — secret scanning in SOP-10's automated gate is the backstop, not the primary control.", controls: ["CRY-09"] },
      { title: "Restrict administrative access to keys", detail: "Non-console administrative access to Key Vault or a database's native encryption configuration requires MFA and goes through Privileged Identity Management, authenticated through the same cryptographic modules the platform uses elsewhere — there is no standing admin session against key material. Encryption is scoped to limit blast radius, not just to satisfy an \"encrypted at rest\" checkbox: keys are separated so that compromising one key doesn't expose every tenant's or every dataset's data, and no single individual can both manage key material and approve their own access to it.", controls: ["CRY-09","CRY-05"] },
  ...
  },
  {
    ...
    domains: ["Change & Cloud Security", "Configuration Management"],
    ...
    steps: [
      { title: "Start from the approved baseline", detail: "Every server, container, or cloud resource is built from an approved, hardened configuration template (CIS Benchmark or vendor-hardened equivalent) — never assembled from scratch with default settings.", controls: ["CFG-02"] },
      ...
      { title: "Route deviations through change control", detail: "Any exception to the baseline requires a ServiceNow change request, reviewed and approved by someone other than the requester; the requester's own access is restricted from approving their own change; and stakeholders who own dependent systems are notified before the change window.", controls: ["CHG-01","CHG-02"] },
      { title: "Vet new cloud/SaaS services before they touch data", detail: "New cloud infrastructure or SaaS tooling goes through IT/Security approval and a Defender for Cloud Apps posture check — including where the provider actually stores and processes data — before it's connected to any ACME data.", controls: ["CLD-01"] },
      { title: "Watch for drift automatically", detail: "Configuration monitoring flags any deviation from baseline after the fact — including unauthorized software installs — so an out-of-process change gets caught instead of quietly persisting.", controls: ["CFG-01"] },
      { title: "Secure cloud APIs and isolate multi-tenant risk", detail: "APIs are authenticated, rate-limited, and reviewed for excessive data exposure before they ship; any shared-tenancy service ACME depends on is evaluated for cross-tenant isolation as part of the cloud/SaaS vetting step above.", controls: ["CLD-04","CLD-06"] },
  ...
  },
  {
    ...
    category: "Detection",
    domains: ["Detection & Monitoring", "Endpoint Security"],
    ...
    steps: [
      ...
      { title: "Capture logs that are actually useful in an investigation", detail: "Forwarded events must include actor identity, a synchronized time stamp, source/destination, and outcome — a log that can't answer \"who did what, when, from where\" doesn't satisfy this step even if it's technically \"logging.\" What gets forwarded is scoped to security-relevant events and their outcomes — authentication, privilege changes, access to Confidential/Restricted data, administrative and configuration actions — not every routine, successful execution of a normal operation; logging everything just buries the signal an investigation actually needs.", controls: ["MON-03"] },
      { title: "Deploy endpoint protection at build time", detail: "Every managed endpoint is enrolled in centralized endpoint device management and runs Microsoft Defender for Endpoint (anti-malware, EDR) and a software firewall before it's allowed to touch ACME data, not bolted on afterward.", controls: ["END-02","END-01"] },
      { title: "Lock down installs and monitor file integrity", detail: "Standard users can't install software without elevation, security tooling itself can't be disabled by a standard user, and host-based intrusion detection plus file integrity monitoring runs on systems handling Confidential/Restricted data so an unauthorized change to a critical file is detected, not discovered later.", controls: ["END-01","END-06"] },
      { title: "Filter phishing and enforce a trusted access path", detail: "Email and endpoint phishing/spam protection is enabled fleet-wide, and login prompts are rendered through a trusted OS-level path so a spoofed credential dialog can't harvest a password.", controls: ["END-08"] },
      { title: "Apply the standard rule set", detail: "The relevant analytics rules (identity, endpoint, network, or cloud) apply automatically by asset category under a documented security concept of operations — no asset waits on custom tuning before it's covered.", controls: ["OPS-01"] },
      { title: "Feed in threat intelligence and track exposure", detail: "External threat intelligence feeds — including a maintained threat catalog relevant to ACME's industry — inform detection rules, and known indicators of exposure (exposed credentials, unpatched internet-facing services) are actively tracked, so ACME is watching for tactics relevant to its actual threat landscape, not a generic rule pack. A vulnerability disclosure program gives outside researchers a channel to report what internal scanning missed.", controls: ["THR-01"] },
      { title: "Run an insider threat program alongside external detection", detail: "Anomalous internal behavior (mass download, off-hours access to Restricted data, access outside a role's normal pattern) is a monitored category in its own right, not an afterthought to external-attacker detection.", controls: ["THR-01"] },
      { title: "Triage against a defined SLA", detail: "SEV1 (active compromise): acknowledge within 15 minutes. SEV2 (high-confidence malicious): 1 hour. SEV3 (suspicious, needs investigation): 4 hours. SEV4 (informational/low-risk): next business day. These are the same severity tiers SOP-12 uses, so triage and response share one clock.", controls: ["OPS-01"] },
      { title: "Escalate or tune", detail: "A confirmed anomaly escalates directly into the SOP-12 Incident Response process at the matching severity tier; a false positive gets tuned back into the rule set so the same noise doesn't repeat; and outbound traffic patterns consistent with covert exfiltration or other anomalous behavior are treated as confirmed until ruled out, not the reverse.", controls: ["MON-01"] },
      { title: "Alert on monitoring failures, not just security events", detail: "A gap in log ingestion or a failed detection pipeline generates its own alert to the SOC — a monitoring blind spot is treated as an incident-worthy condition, not silently absorbed.", controls: ["MON-02"] },
      { title: "Retain and protect logs", detail: "Security logs are retained a minimum of 12 months per ACME's log retention schedule and write-protected — an investigation can't rely on logs that could've been altered or already aged out.", controls: ["MON-08"] },
      { title: "Review signal quality quarterly", detail: "Alert volume and false-positive rate are reviewed quarterly so the monitoring stays usable instead of becoming noise everyone tunes out.", controls: ["MON-01"] },
      ...
      { title: "Watch for API-specific abuse patterns", detail: "Beyond the standard anomaly rules, API traffic is watched for patterns specific to how APIs get abused: repeated 401/403 responses, sequential object-ID enumeration, unusual bulk-export volume, activity against a deprecated version or endpoint, repeated rate-limit violations, and unexpected service-to-service call patterns. A confirmed pattern escalates into SOP-12 at the matching severity, the same as any other anomaly.", controls: ["MON-01"] },
    ],
  },
  {
    ...
    steps: [
      ...
      { title: "Gate access on device trust", detail: "A device must pass an Intune compliance check (screen lock, encryption, current OS) before it's allowed to reach ACME data, whether company-owned or BYOD.", controls: ["MDM-01"] },
      {
        ...
        evidence: {
          ...
          reconcile: { formula: "reviewed = approved + flagged", check: (v: Record<string, string | number | undefined>) => Number(v.reviewed) === Number(v.approved) + Number(v.flagged) },
          summaryKey: "flagged",
        },
      },
      {
      ...
      { title: "Enforce session and credential hygiene", detail: "Sensitive actions require re-authentication even within an active session; credentials are never shared between users; accounts lock out after repeated failed attempts; and idle sessions terminate automatically rather than staying open indefinitely.", controls: ["IAC-20","IAC-10"] },
      { title: "Extend the same identity standard to third parties", detail: "A contractor or third-party integration authenticates through the same centralized identity path as an employee, and a new identity — human or service account — is proofed before it's trusted, not created on request alone.", controls: ["IAC-07"] },
  ...
  },
  {
    ...
    category: "Resilience",
    domains: ["Business Continuity", "Capacity & Performance"],
    policyId: "business-continuity",
    owner: "IT / Engineering",
    reviewCadence: "Restore tested at least annually for Tier 1 assets",
    evidenceType: "Screenshot",
    purpose: "How ACME keeps an asset recoverable and adequately resourced — backups, replication, and capacity headroom, proven rather than assumed. Scheduled maintenance windows are owned by SOP-13 with the rest of Physical & Maintenance.",
    steps: [
      { title: "Assign a recovery tier at onboarding", detail: "Every business-critical asset gets an RPO/RTO target at onboarding, tied to how it was identified as a critical asset in the first place under the Business Continuity Management System — recovery expectations are set before there's ever an incident, not during one.", controls: ["BCD-01"] },
      { title: "Configure scheduled, isolated backups", detail: "Veeam backs up the asset on a schedule matching its tier, encrypted and stored separately from production so one failure can't take out both copies; backup hardware and media are physically protected to the same standard as production. Backup storage uses immutable, write-once retention or a credential and access path separate from production, so that fully compromising production does not also give an attacker the ability to delete or encrypt the recovery copies.", controls: ["BCD-11"] },
      { title: "Replicate Tier 1 workloads", detail: "Tier 1 Azure workloads replicate to an alternate storage site in a different region via Azure Site Recovery, with alternative security measures and telecommunications availability confirmed for that site — a standby site nobody has verified can actually run production isn't a real standby site.", controls: ["BCD-08"] },
      { title: "Actually test the restore", detail: "Restoration is tested at least annually for Tier 1 assets — a backup that's never been restored isn't a proven backup, it's an assumption. The test includes recovering the service itself, not just retrieving files, so reconstitution is proven rather than hoped for.", controls: ["BCD-04", "BCD-12"] },
      { title: "Watch capacity continuously", detail: "Azure Monitor tracks capacity continuously against defined thresholds, and resource priority is set in advance for degraded-capacity scenarios so resilience doesn't quietly erode because something simply ran out of room.", controls: ["CAP-01"] },
      { title: "Capture lessons and update the plan", detail: "Every test or real disruption produces a documented root cause analysis and lessons learned, fed back into this procedure — not filed away and forgotten.", controls: ["BCD-04"] },
    ],
  },
  {
    ...
    steps: [
      { title: "Log the processing activity", detail: "Every new use of personal data — including any use of health-related data shared in a support interaction — is recorded in ACME's Record of Processing Activities (RoPA) with its purpose and legal basis before it starts, not after.", controls: ["PRI-01"] },
      { title: "Publish what's collected and why", detail: "ACME's privacy notice is kept current with what's actually collected and processed, including how choice and consent are captured where required — it describes the real RoPA, not a generic template.", controls: ["PRI-02"] },
      ...
      { title: "Keep data quality current", detail: "A correction accepted from a data subject or patient updates the source system through a defined update process, not just a note in a ticket — RoPA accuracy and disclosure accounting both depend on the underlying data actually being right.", controls: ["PRI-04"] },
      { title: "Escalate a privacy incident with its own notification clock", detail: "A privacy incident — including one involving health-related data — is escalated through SOP-12 like any other incident, but Legal and Privacy separately track any statutory notification deadline that applies (for a health-data breach, notification to affected individuals and regulators within 60 days), and the response is reviewed and tested as part of ACME's ongoing privacy control testing and training.", controls: ["PRI-01"] },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Layer defenses at the boundary", detail: "The network perimeter uses multiple independent network security controls (firewall, IPS, proxy) rather than a single control an attacker only has to beat once.", controls: ["NET-01","NET-03"] },
      { title: "Segment production from corporate and guest networks — and verify it", detail: "Access control lists enforce data-flow restrictions between segments to documented business need. Segmentation is verified by an annual internal test that specifically attempts cross-segment lateral movement — a network diagram showing segments isn't evidence that they actually hold.", controls: ["NET-06"] },
      { title: "Formalize connections to outside networks", detail: "Any direct network connection to a partner, vendor, or acquired company's network is governed by a documented Interconnection Security Agreement before traffic flows, not set up informally by whoever has the fastest path to get it working.", controls: ["NET-01"] },
      { title: "Filter DNS and content at the edge", detail: "Zscaler enforces DNS-layer filtering against known-malicious destinations for every managed device, on or off the corporate network.", controls: ["NET-17"] },
      { title: "Protect data and session integrity over open networks", detail: "Traffic carrying Confidential/Restricted data over any open or untrusted network is encrypted end-to-end, sessions are protected against hijacking or replay, and idle or stale connections are terminated rather than left open indefinitely — not just authenticated once at connection time.", controls: ["NET-01","NET-14"] },
      { title: "Secure wireless access", detail: "Corporate wireless requires certificate- or credential-based authentication with the same encryption standard as SOP-01's transit requirement — an open or WEP/WPA-only network is never in scope for ACME data.", controls: ["NET-14"] },
      { title: "Put public-facing web apps behind a WAF", detail: "Customer-facing applications sit in a DMZ behind Cloudflare's WAF and CDN with TLS enforced end-to-end for all web traffic, are monitored for unauthorized website changes, and are never exposed directly.", controls: ["WEB-01","WEB-03"] },
      { title: "Require strong authentication on customer-facing apps", detail: "Customer-facing applications handling Confidential/Restricted data enforce strong customer authentication (not a password alone) before granting account access." , controls: ["WEB-03"] },
      { title: "Inspect outbound traffic for data loss", detail: "Microsoft Purview DLP inspects outbound email, electronic messaging, and file transfers for Confidential/Restricted data leaving through approved channels.", controls: ["NET-17"] },
      { title: "Monitor for intrusion at egress", detail: "Network intrusion detection is deployed at ACME's internet egress and monitored by Security Operations, with alerts triaged against the same SEV1–SEV4 SLA table SOP-03 defines.", controls: ["NET-01"] },
    ],
  },
  {
    ...
    steps: [
      { title: "Register the asset before it goes live", detail: "Every technology asset, application, and data store is added to ACME's asset system of record with a named business owner before it's used for real work.", controls: ["AST-01"] },
      { title: "Maintain network and data-flow diagrams as the estate changes", detail: "Network diagrams and data flow diagrams are updated whenever an asset materially changes what it connects to, not redrawn once a year from memory — the reconciliation step below checks the diagram against reality, not just the inventory list.", controls: ["AST-02"] },
      { title: "Set expectations for unattended and public-facing equipment", detail: "Devices are locked when unattended, and any kiosk or point-of-interaction device (e.g. a physical check-in or payment terminal) is hardened and tamper-evident by design, not treated as a normal endpoint." , controls: ["AST-05"]},
      { title: "Govern personal devices used for work", detail: "A personal device used for ACME work is tracked as an asset for acceptable-use purposes even though ACME doesn't own it — this is the inventory-side counterpart to SOP-04's device-trust access control, not a duplicate of it.", controls: ["AST-05"] },
      { title: "Protect against logical tampering", detail: "Firmware and configuration integrity on critical assets is checked so an unauthorized modification below the OS layer doesn't go unnoticed.", controls: ["AST-05"] },
      ...
      { title: "Confirm the asset comes back on offboarding", detail: "A departing employee's issued hardware is returned and logged before final settlement, coordinated with SOP-14's same-day offboarding trigger — access being revoked doesn't automatically mean the laptop came back.", controls: ["AST-09"] },
  ...
  },
  {
    ...
    steps: [
      { title: "Design in defense-in-depth from the start", detail: "New systems are architected with multiple layers of protection under a documented defense-in-depth architecture, aligned to ACME's target enterprise architecture, not a single control relied on alone or a one-off design nobody else's systems match.", controls: ["SEA-01","TDA-01"] },
      { title: "Isolate processes and plan for failure", detail: "Processes run isolated from each other by default, and new systems go through a predictable-failure analysis (what happens when a dependency is unavailable) before launch, not after the first outage reveals it.", controls: ["SEA-01"] },
      { title: "Harden login and don't leak system details", detail: "Login flows go through OS/platform-secure log-on procedures, and error responses, headers, and banners don't reveal stack traces, versions, or internal architecture to an unauthenticated caller.", controls: ["SEA-01"] },
      { title: "Keep clocks synchronized", detail: "All systems sync to a common time source — log correlation and forensic timelines across SOP-03 and SOP-12 depend on timestamps actually agreeing with each other.", controls: ["SEA-01"] },
      { title: "Secure the development environment itself", detail: "Development environments are built and configuration-managed to the same rigor as production — a dev environment with weak controls is a viable path to production credentials and code.", controls: ["TDA-06"] },
      { title: "Separate dev, test, and production", detail: "Real customer data isn't used in lower environments without an approved, documented exception.", controls: ["TDA-06", "TDA-18"] },
      { title: "Screen developers before granting production access", detail: "An engineer gets access to production code or data only after the role-appropriate background screening SOP-14 requires is complete — access isn't granted on start date by default." , controls: ["TDA-20"]},
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Commission independent penetration testing", detail: "A qualified third party tests production environments at least annually, and findings feed the same remediation process as any other vulnerability.", controls: ["VPM-07"] },
  ...
  },
  {
    ...
    steps: [
      { title: "Report immediately, don't investigate alone", detail: "A suspected incident is reported to IT Security right away; the reporting employee isn't expected to triage it themselves, and Security provides a clear, low-friction way for anyone — employee, customer, or vendor — to report a suspected issue.", controls: ["IRO-02", "IRO-10"] },
      { title: "Classify by severity", detail: "Incidents are tiered SEV1–SEV4 — the same tiers SOP-03 triages alerts against — with response timelines and escalation paths that match the tier, not a one-size-fits-all response.", controls: ["IRO-01"] },
      { title: "Determine notification obligations", detail: "Legal and Privacy determine whether customers, regulators, or individuals must be notified, and on what timeline, per law or contract — including the accelerated timeline for a data spill involving sensitive or regulated data.", controls: ["IRO-10", "IRO-02"] },
      { title: "Follow chain of custody for forensic evidence", detail: "Evidence collection follows a documented chain-of-custody process so it stays admissible if needed.", controls: ["IRO-02"] },
      { title: "Train responders and test the plan annually", detail: "ISIRT members complete incident response training, and the IR plan itself is exercised at least annually through a tabletop or live drill — a plan that's only ever been read, not run, isn't a tested plan.", controls: ["IRO-04", "IRO-06"] },
  ...
  },
  {
    ...
    steps: [
      { title: "Badge in individually", detail: "Every entry uses the individual's own badge, authorized in advance against a defined access list under ACME's physical access control program; tailgating is refused, not waved through.", controls: ["PES-03"] },
      { title: "Escort every visitor", detail: "Guests sign in, wear a visible visitor badge, and are escorted in non-public areas at all times under a documented visitor control process — an unescorted guest is treated as a security event, not an oversight.", controls: ["PES-01", "PES-03"] },
      { title: "Require extra clearance for sensitive areas", detail: "Server and network rooms require authorization beyond a standard building badge, and every entry is logged and monitored, not just recorded.", controls: ["PES-03"] },
      { title: "Control deliveries and equipment removal", detail: "Deliveries are received at a controlled dock, logged, and inspected before entering secure areas; equipment leaving the building requires a documented asset-removal authorization, tying back to SOP-09's asset record.", controls: ["PES-03"] },
      { title: "Monitor environmental conditions continuously", detail: "Server rooms and other equipment sites have fire suppression, temperature/humidity monitoring, backup power, and equipment sited to reduce environmental risk, monitored continuously against defined thresholds under ACME's physical and environmental protection program, not checked periodically.", controls: ["PES-01"] },
      { title: "Review access lists quarterly", detail: "Access lists for restricted areas are reviewed quarterly by the area owner against the physical access monitoring log, the same cadence as a standard access recertification; anyone who no longer needs access is removed within 5 business days of the review.", controls: ["PES-03"] },
      { title: "Extend physical security to remote work locations", detail: "A remote employee's home office is expected to meet baseline physical security: Restricted-tier printouts and devices are stored where household members or visitors can't access them, and a work laptop isn't left logged in and unattended in a shared space.", controls: ["PES-03"] },
      { title: "Shield sensitive facilities from signal emanation", detail: "Facilities processing highly sensitive data (data centers, executive briefing areas) are assessed for electromagnetic signal leakage — including electromagnetic pulse exposure — that could expose information outside the physical boundary, and shielded where the risk warrants it.", controls: ["PES-01"] },
      { title: "Schedule maintenance in defined windows", detail: "Patching and maintenance happen in communicated windows using approved maintenance tools, on a timeliness standard matched to the finding's severity, not ad hoc, so a routine task never gets mistaken for an outage.", controls: ["MNT-01", "MNT-05"] },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Put security expectations in writing", detail: "Offer and onboarding paperwork include acknowledgment of ACME's security and acceptable use policies.", controls: ["HRS-04"] },
      ...
      { title: "Reassess access on role change", detail: "A transfer or promotion triggers an access review; old access doesn't carry forward by default.", controls: ["HRS-09"] },
      ...
      { title: "Apply consistent sanctions for policy violations", detail: "A confirmed violation of security policy goes through a defined, documented progressive-discipline process — the consequence for the same violation doesn't depend on who's managing the employee." , controls: ["HRS-09"]},
      { title: "Extend screening and terms to contractors", detail: "Contractors and third-party personnel working under ACME's badge or systems get the same screening and written acknowledgment requirements as employees — a staffing-agency hire isn't exempt because they're not on ACME's own payroll.", controls: ["HRS-09"] },
      ...
      { title: "Train everyone annually, and track it", detail: "All employees complete security awareness training at hire and annually thereafter through KnowBe4, building the security-minded workforce the program depends on, with completion tracked, not assumed.", controls: ["SAT-02"] },
      { title: "Run phishing simulations as practice", detail: "Simulated phishing emails give employees a chance to practice spotting real ones; results are logged to each employee's training record, and repeat misses get targeted coaching, not just a note in a file.", controls: ["SAT-02"] },
    ],
  },
  {
    ...
    steps: [
      ...
      { title: "Put security terms in the contract", detail: "Data processing agreements, breach notification timelines, and right-to-audit clauses are standard terms for vendors handling Confidential/Restricted data.", controls: ["TPM-04"] },
      { title: "Scope and review vendor access", detail: "Vendor personnel are screened to the same personnel-security standard as ACME contractors under SOP-14 and get minimum necessary access, reviewed on the same cadence as employee access under SOP-04." , controls: ["TPM-01", "TPM-04"]},
      ...
      { title: "Treat a material change as an off-cycle trigger", detail: "A vendor acquisition, a subprocessor change, or a scope-of-service change triggers an off-cycle reassessment immediately, rather than waiting for the next annual or biennial cycle.", controls: ["TPM-08"] },
      ...
      { title: "Consider the vendor's own supply chain", detail: "A critical vendor's sub-processors and dependencies are factored into the risk assessment under ACME's supply chain risk management plan, not treated as out of scope.", controls: ["TPM-01"] },
  ...
  },
  {
    ...
    category: "Governance",
    domains: ["AI & Autonomous Technologies"],
    ...
    // Steps are grouped by security intent rather than authored Required-first
    // like the other 15 SOPs — with 24 steps and only 11 individually
    // control-cited, a flat Required/then/Recommended split buried the "why"
    // behind two-thirds of the procedure. Groups are ordered by how many
    // crosswalk controls they carry (see each group's steps), ties broken by
    // step count. Required/Recommended is still shown per step (still a real,
    // crosswalk-derived distinction — see ControlMapping) — it's just no
    // longer the axis the reader scans top to bottom by.
    stepGroups: [
      { id: "lifecycle", title: "AI Systems Are Governed Across Their Lifecycle", blurb: "AI inventory, model/version change management, model/provider reviews, supply-chain security, model artifact integrity, training-data provenance, poisoning controls, continuous testing/red teaming, regression testing, third-party model risk, and periodic reassessment." },
      { id: "observability", title: "AI Actions Are Observable and Containable", blurb: "Logging, traceability, runtime anomaly detection, tool-call telemetry, incident response, kill switches, credential revocation, disabling tools/models/RAG sources, and forensic reconstruction." },
      { id: "boundary", title: "The Model Is Never the Security Boundary", blurb: "Authorization outside the model, credential isolation, policy enforcement, tenant scoping, RAG authorization, MCP/tool authorization, and human approval for high-impact actions." },
      { id: "capabilities", title: "Agent Capabilities Are Minimized and Constrained", blurb: "Least privilege, restricted tools, scoped IAM, egress restrictions, execution sandboxing, resource quotas, loop limits, SSRF protections, network isolation, and bounded autonomy." },
      { id: "context", title: "All AI Context Is Untrusted", blurb: "Prompt injection, indirect prompt injection, malicious RAG content, poisoned memory, hostile web/email/tool content, system-prompt exposure assumptions, and input filtering." },
      { id: "output", title: "Model Output Is Untrusted Until Validated", blurb: "Structured output validation, schema enforcement, SQL/command/API validation, output sanitization, safe rendering, independent verification, and high-impact decision checks." },
    ],
    steps: [
      // --- AI Systems Are Governed Across Their Lifecycle (13 controls, 8 steps) ---
      { group: "lifecycle", title: "Inventory every AI system before it's relied on", detail: "Every AI model or agent ACME builds or deploys — and every third-party AI tool approved for employee use — is tracked with an owner, business purpose, risk tier, and lifecycle status. The entry also captures the provider/model version, the data it touches, whether it uses RAG or persistent memory, which tools it can call, and its next review date. An AI system with no inventory entry isn't an approved AI system.", controls: ["AAT-01","AAT-07"] },
      { group: "lifecycle", title: "Approve the use case before build, not after ship", detail: "A new AI feature or workflow goes through the same governance gate as any other initiative (SOP-05), plus AI-specific questions: what data will it see, what tools can it call, and is this a use case ACME has actually approved — not one discovered already running in production.", controls: ["AAT-01"] },
      { group: "lifecycle", title: "Track training, fine-tuning, and retrieval data provenance — and guard against poisoning", detail: "Data used to fine-tune, embed, or retrieve into an AI system's context is traced to its classification tier under SOP-01, and ingested only from authorized sources with restricted write access and an audit trail. Being inside an internal system doesn't make a document trustworthy by default — content is re-indexed after a source changes, and a compromised source can be rolled back without rebuilding the whole index.", controls: ["AAT-17"] },
      { group: "lifecycle", title: "Convene a diverse review board for high-impact systems", detail: "Before a high-impact AI system launches, its review includes stakeholders beyond the engineering team building it — Legal, Privacy, and the affected business unit — to catch fairness, reliability, and safety issues a single team's perspective would miss.", controls: ["AAT-17", "AAT-16"] },
      { group: "lifecycle", title: "Protect AI artifacts across their lifecycle", detail: "A foundation model, downloaded weights, a fine-tune, an inference runtime, or an MCP server is a supply-chain artifact like any other — pulled from a trusted, version-pinned source, verified against a signature, and scanned for vulnerabilities before it's trusted. For models ACME builds itself, those artifacts are protected as high-value assets: restricted access, monitored transfers, and encrypted storage — applicability-based, so an ordinary SaaS API consumer isn't expected to build export-monitoring for a model it never downloads.", controls: ["AAT-17"] },
      { group: "lifecycle", title: "Test for prompt injection and adversarial misuse — continuously, not just once", detail: "Any AI system that accepts user input is tested for prompt injection and jailbreak attempts before go-live, the same way a web app gets tested for injection under SOP-10. That testing is repeatable, not a one-time gate — scenarios like RAG poisoning, tool abuse, and cross-tenant retrieval are re-run after a material change, and previously-fixed behaviors are kept as regression tests so an upgrade can't silently reintroduce them.", controls: ["AAT-10"] },
      { group: "lifecycle", title: "Re-validate on material AI or model change", detail: "AI systems can change materially without a line of ACME's own code changing — a model upgrade, a new provider, a new tool, or a provider silently changing a hosted model's behavior. Any of these triggers a re-evaluation proportional to the risk of the change, and where possible the specific model and version in production is recorded so an issue can be traced to what produced it.", controls: ["AAT-10"] },
      { group: "lifecycle", title: "Review the model or provider like any other vendor", detail: "A third-party model provider goes through SOP-15's vendor risk process, with AI-specific questions added: does it train on ACME's data by default, how long are prompts retained, and where is data processed. Classification set under SOP-01 travels with the data through the model call the same as any other system, and secrets are never placed in a prompt." },

      // --- AI Actions Are Observable and Containable (3 controls, 3 steps) ---
...
export const PROCEDURES: AuthoredProcedure[] = PROCEDURE_DEFS.map((p) => {
