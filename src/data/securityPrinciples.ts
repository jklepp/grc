...
export type PrincipleStatus = "operationalized" | "partial" | "not-operationalized";
...
export interface PrincipleStepReference {
...
interface PrincipleDefinition {
  ...
  rationale: string;
  tags: string[];
  implementedIn: PrincipleStepReference[];
  partial?: boolean;
  partialNote?: string;
  gapNote?: string;
}

interface PrincipleDomainDefinition {
  id: string;
  title: string;
  summary: string;
...
export interface ResolvedPrincipleStep extends PrincipleStepReference {
  procedureId: string;
  procedureTitle: string;
  controlIds: string[];
}

export interface SecurityPrinciple extends PrincipleDefinition {
  implementedIn: ResolvedPrincipleStep[];
  controlIds: string[];
...
export const FOUNDATIONAL_PRINCIPLES: FoundationalPrinciple[] = [
...
const PRINCIPLE_DOMAIN_DEFS: PrincipleDomainDefinition[] = [
  {
...
function resolveStepRef(domainId: string, principleId: string, { procedureCode, step: stepTitle }: PrincipleStepReference) {
...
export const PRINCIPLE_DOMAINS: PrincipleDomain[] = PRINCIPLE_DOMAIN_DEFS.map((domain) => ({
  ...domain,
  principles: domain.principles.map((p) => {
    ...
    const status: PrincipleStatus = implementedIn.length === 0 ? "not-operationalized" : p.partial ? "partial" : "operationalized";
...
export const PRINCIPLE_STATUS_COUNTS = PRINCIPLE_DOMAINS.reduce(
  ...
  { operationalized: 0, partial: 0, "not-operationalized": 0, total: 0 } as Record<PrincipleStatus | "total", number>
);
