...
const PROCEDURE_DEFS = [
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Capture logs that are actually useful in an investigation", detail: "Forwarded events must include actor identity, a synchronized time stamp, source/destination, and outcome — a log that can't answer \"who did what, when, from where\" doesn't satisfy this step even if it's technically \"logging.\"", controls: ["MON-03", "MON-07"] },
  ...
  },
  {
    ...
    steps: [
      { title: "Provision through Entra ID/Okta only", detail: "No asset gets a local account, a shared credential, or an access path that bypasses centralized identity — ever.", controls: ["IAC-01", "IAC-07"] },
      { title: "Grant least privilege at request time", detail: "Access is scoped to what the role requires through role-based access control and approved by the requester's manager before it's granted, not adjusted down after the fact.", controls: ["IAC-08", "IAC-20"] },
      ...
      { title: "Recertify on a fixed cadence", detail: "Privileged access is recertified quarterly, standard access semi-annually. Access that isn't reconfirmed by its owner is revoked, not left standing by default.", controls: ["IAC-16", "IAC-17"] },
      { title: "Revoke same-day on offboarding", detail: "An HR termination or role-change event triggers same-business-day access revocation and remote device wipe, not a queued ticket.", controls: ["IAC-07", "MDM-05"] },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Build, stage, and gate production separately", detail: "A merged change builds an artifact, deploys to staging, runs dynamic testing (DAST) against the running application, and requires a separate production-deployment approval — merge and production release are two different events, not one." },
