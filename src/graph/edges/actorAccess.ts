...
// in), outbound (one of our assets calls out to it, e.g. a third-party
// model provider), or internal (the actor operates the boundary directly —
// a platform administrator with standing reach into both the data plane and
// control plane, rather than a request-path call in either direction) —
// because that's a real fact worth keeping even though inbound and outbound
// render together in the same Actors stage rather than being split into
// separate categories on the diagram. `assetId` is whichever
...
export const ACTOR_DIRECTIONS = { INBOUND: "inbound", OUTBOUND: "outbound", INTERNAL: "internal" } as const;
