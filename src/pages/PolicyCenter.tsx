...
import type { AuthoredPolicy, PolicyClassificationLevel } from "../data/policies";
...
function CoreBadge({ compact = false }: { compact?: boolean }) {
  return (
    <span
...
function ControlMapping({ policy }: { policy: AuthoredPolicy }) {
  return (
...
function ClassificationLevels({ levels, onViewRegister }: { levels: PolicyClassificationLevel[]; onViewRegister?: () => void }) {
  const ordered = CLASS_ORDER.map((lvl) => levels.find((level) => level.level === lvl)).filter((level): level is PolicyClassificationLevel => level !== undefined);
  return (
    <div className="mb-6">
...
export default function PolicyCenter({ onNavigate }: { onNavigate?: (target: string) => void }) {
  ...
  const relatedPolicies = (selected.relatedPolicyIds || [])
    .map((id) => POLICIES.find((policy) => policy.id === id))
    .filter((policy): policy is AuthoredPolicy => policy !== undefined);
