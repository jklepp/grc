...
function meta(entry, colorKey, bgKey) {
  return Object.defineProperties({ ...entry }, {
    color: { get: () => C[colorKey], enumerable: true },
    bg: { get: () => C[bgKey], enumerable: true },
  });
}
...
export const EVIDENCE_HEALTH_ORDER = ["missing", "stale", "weak", "adequate", "strong"];

export function evidenceHealthForRow(row) {
  const evidence = (row.instances ?? []).flatMap((i) => i.evidence ?? []);
  if (evidence.length === 0) {
    return { level: "missing", ...EVIDENCE_HEALTH_META.missing, detail: null };
  }
