...
import type { ProfileEvaluationStatus } from "../engine/profile";
import type { AssuranceCategory, ClassificationTier } from "../graph/nodes/taxonomy";

const SYSTEMS = getAllSystems();

type ProfileColor = "green" | "amber" | "red";

function colorFor(key: ProfileColor): { color: string; bg: string } {
  const colors = {
...
const STATUS_META: Record<ProfileEvaluationStatus, { label: string; key: ProfileColor; Icon: typeof Check }> = {
  met: { label: "Met", key: "green", Icon: Check },
  partial: { label: "Partial", key: "amber", Icon: Minus },
  gap: { label: "Gap", key: "red", Icon: XIcon },
};

function TierTab({ tier, active, onSelect }: { tier: ClassificationTier; active: boolean; onSelect: (tier: ClassificationTier) => void }) {
...
type ProfileRequirement = (typeof CONTROL_PROFILES)[ClassificationTier][AssuranceCategory];

function RequirementCard({ category, requirement }: { category: AssuranceCategory; requirement: ProfileRequirement }) {
  return (
...
function StatusPill({ status }: { status: ProfileEvaluationStatus }) {
...
function SystemComplianceRow({ system }: { system: SystemRollup }) {
  ...
  const target = tierTargetScore(system.classification ?? "Internal");
  return (
...
export default function ControlProfile() {
  const [tier, setTier] = useState<ClassificationTier>(CLASS_ORDER[CLASS_ORDER.length - 1]);
