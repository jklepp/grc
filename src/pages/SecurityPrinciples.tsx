...
import type { FoundationalPrinciple, PrincipleDomain, PrincipleStatus, SecurityPrinciple } from "../data/securityPrinciples";
...
const STATUS_COLOR: Record<PrincipleStatus, () => string> = {
  operationalized: () => C.green,
  partial: () => C.amber,
  "not-operationalized": () => C.red,
};

const FOUNDATIONAL_BY_ID: Record<string, FoundationalPrinciple | undefined> = Object.fromEntries(FOUNDATIONAL_PRINCIPLES.map((f) => [f.id, f]));
const STATUS_ORDER = ["operationalized", "partial", "not-operationalized"] as const satisfies readonly PrincipleStatus[];
...
function PrincipleCard({ principle, domain, onNavigate, showDomainLabel = false }: PrincipleCardProps) {
...
export default function SecurityPrinciples({ onNavigate }: { onNavigate?: (target: string) => void }) {
  const [selectedDomainId, setSelectedDomainId] = useState(PRINCIPLE_DOMAINS[0].id);
  const [activeStatus, setActiveStatus] = useState<PrincipleStatus | null>(null);
